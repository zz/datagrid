...
export function buildChartData(rows: Value[][], labelIndex: number, valueIndex: number, limit = 100): ChartPoint[] {
    return rows.slice(0, limit).flatMap((row, index) => {
        ...
        return [{ label: labelCell ? displayValue(labelCell) : String(index + 1), value }]
    })
}
