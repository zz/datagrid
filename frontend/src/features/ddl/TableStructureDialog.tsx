...
// table's columns and indexes in one place. Each change applies immediately and
// the affected list refreshes, rather than batching a diff on OK.
export default function TableStructureDialog({
    ...
    // Per-row edit state.
    const [renameCol, setRenameCol] = useState<{ from: string; to: string } | null>(null)
    ...
    // Run a mutation, refresh both lists + the tree, surfacing any error.
    ...
    const commitRename = () => {
        if (!renameCol) return
        const { from, to } = renameCol
        if (!to.trim() || to === from) {
            setRenameCol(null)
            return
        }
        run(() => RenameColumn(connId, schema ?? '', table, from, to.trim()), () => setRenameCol(null))
    }
                    ...
                    {info === null ? (
                        ...
                        info.columns.map(c => (
                            <div className="struct-row" key={c.name}>
                                {renameCol?.from === c.name ? (
                                    <>
                                        <input
                                            autoFocus
                                            className="struct-rename"
                                            value={renameCol.to}
                                            onChange={e => setRenameCol({ from: c.name, to: e.target.value })}
                                            onKeyDown={e => {
                                        ...
                                        <button onClick={() => setRenameCol(null)}>Cancel</button>
                                    </>
                                ) : (
                                    <>
                                        <span className="struct-col-name">{c.name}</span>
                                        <span className="struct-col-type">{c.typeName}</span>
                                        {pk.has(c.name) && <span className="index-badge">PK</span>}
                                        ...
                                        {dropCol === c.name ? (
                                            ...
                                            <>
                                                <button className="icon-btn" title="Rename column" disabled={readOnly || busy} onClick={() => setRenameCol({ from: c.name, to: c.name })}>
                                                    ✎
                                                </button>
